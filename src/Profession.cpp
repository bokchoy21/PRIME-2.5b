...
static const char *
filterIntro(const char *instr)
{
    return processLoreLine(instr, NULL);
}

static void
showIntro (const char *filename)
{
    shTextViewer *intro = new shTextViewer (filename, INTRODIR, filterIntro);
    if (intro->hasText ())
        intro->show ();
    delete intro;
}
...
{
    if (obj->isA (kFloppyDisk) and !obj->is (obj::examined))
    {
        obj->set (obj::examined);
        if (RNG (2))
        {
...
chooseProfession ()
{
    ...
    shMenu *menu = I->newMenu ("Choose your profession (TAB for descriptions)", shMenu::kNoHelp);
