...
repairObject (shCreature *user, shObject *tool)
{
    ...
    if (RNG(1, 20) + user->getSkillModifier (kRepair) > 15) {
        obj->mDamage = 0;
...
shCreature::useKey (shObject *key, shFeature *door)
{
    ...
    if (cardneeded and key->has_subtype (keycard)) {
        ...
        int score = RNG(1, 20) + getSkillModifier (kOpenLock);
            ...
            for (int i = 0; i < v->count (); ++i) {
                ...
                if (obj->is (obj::known_type) and
                    (!(obj->isA (kObjBrain) or obj->isA (kObjBioArmor) or
                       obj->isA (kObjBioComputer) or obj->isA (kObjUngooma)) or
                ...
                if (obj->isA (kObjBrain) or obj->isA (kObjBioArmor) or
                    obj->isA (kObjBioComputer) or obj->isA (kObjUngooma))
                {
...
useRemote (shCreature *user, shObject *remote)
{
    ...
    shFeature *f = user->mLevel->getFeature (x, y);
    if (!c and !f) {
        I->p ("Nothing happens.");
        return elapsed;
    }
...
*/
    ...
    if (!(is_eligible or will_react) or !is_in_range) {
...
*/
    if (f and will_react) {
        ...
        if (f->isOpenDoor ())
            ...
            if (see and f->isHiddenDoor ())
                I->p ("A part of wall reacts!");

            f->mType = shFeature::kDoorOpen;
        }
        f->mDoor.mMoveTime = Clock;
        user->mLevel->computeVisibility (user);
...
swallowBluePill (shCreature *user, shObject *pill)
{
    if (Level->isMainframe ()) {
        I->p ("This will not change anything.");
        return 0;
    }
