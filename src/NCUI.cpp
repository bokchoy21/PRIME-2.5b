...
shNCursesInterface::showVersion ()
{
    ...
    newWin (kTemp);
    moveWin (kTemp, 10, 0, 10 + 60, 0 + 18);

    for (int y = 0; y < 8; ++y) {
        for (int x = 0; x < len; ++x) {
            if (splash_logo[y][x] == '#') {
                wattrset (mWin[kTemp], ColorMap[kLime]);
            } else {
                wattrset (mWin[kTemp], ColorMap[kGreen]);
            }
            mvwaddnstr (mWin[kTemp], y, x + 15, &splash_logo[y][x], 1);
        }
    }
    ...
    mvwaddstr (mWin[kTemp], 9, 7,  "P");
    mvwaddstr (mWin[kTemp], 9, 13, "R");
    mvwaddstr (mWin[kTemp], 9, 25, "I");
    mvwaddstr (mWin[kTemp], 9, 39, "M");
    mvwaddstr (mWin[kTemp], 9, 44, "E");
    wattrset (mWin[kTemp], ColorMap[kOrange]);
    mvwaddstr (mWin[kTemp], 11, 14, "Unofficial variant of ZAPM v" ZAPM_VERSION " ");
    mvwaddstr (mWin[kTemp], 12, 14, "by Psiweapon and Michal Bielinski");
    wattrset (mWin[kTemp], ColorMap[kRed]);
    mvwaddstr (mWin[kTemp], 13, 17, "http://pesky-reticulans.org");
    wattrset (mWin[kTemp], A_NORMAL);
    mvwaddstr (mWin[kTemp], 15, 15, "ZAPM (C) 2002-2010 Cyrus Dolph.");
    mvwaddstr (mWin[kTemp], 16, 20, "All rights reserved.");
    mvwaddstr (mWin[kTemp], 17, 20, "http://www.zapm.org");
    wattrset (mWin[kTemp], ColorMap[kLime]);
    wattrset (mWin[kTemp], A_NORMAL);
...
void
shNCursesInterface::hideVersion ()
{
    delWin (kTemp);
}
